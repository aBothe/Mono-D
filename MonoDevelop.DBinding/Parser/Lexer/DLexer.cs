...
namespace MonoDevelop.D.Parser.Lexer
{
    ...
    public abstract class AbstractLexer
    {
        ...
        public void SetInitialLocation(DomLocation location)
        {
        ...
        protected DomLocation lastLineEnd = new DomLocation(1, 1);
        protected DomLocation curLineEnd = new DomLocation(1, 1);
        protected void LineBreak()
        {
            lastLineEnd = curLineEnd;
            curLineEnd = new DomLocation(line,col - 1);
        }
    ...
    public class DLexer : AbstractLexer
    {
        ...
        void ReadComment()
        {
            switch (ReaderRead())
            {
                case '+':
                    if (ReaderPeek() == '+')// DDoc
                        ReadMultiLineComment(CommentType.MultiLine,true, true);
                    else
                        ReadMultiLineComment(CommentType.MultiLine, true,false);
                    break;
                case '*':
                    if (ReaderPeek() == '*')// DDoc
                        ReadMultiLineComment(CommentType.MultiLine, false,true);
                    else
                        ReadMultiLineComment(CommentType.MultiLine, false,false);
                    break;
                case '/':
                    if (ReaderPeek() == '/')// DDoc
                        ReadSingleLineComment(CommentType.SingleLine,true);
                    else
                        ReadSingleLineComment(CommentType.SingleLine,false);
                    break;
                default:
        ...
        void ReadSingleLineComment(CommentType commentType,bool isDocumentation)
        {
            var st = new DomLocation(Line,Col);
            string comm = ReadToEndOfLine().TrimStart('/');
            var end = new DomLocation(st.Line,Col);
			var nComm = new Comment(comm.Trim()) {
				CommentType=commentType, 
				Region=new DomRegion(st,end), 
				CommentStartsLine=st.Column<2, 
				IsDocumentation=isDocumentation};

            Comments.Push(nComm);
        }

        void ReadMultiLineComment(CommentType commentType, bool isNestingComment, bool isDocumentation)
        {
            int nextChar;
            Comment nComm = null;
            var st = new DomLocation(Line,Col);
            var scCurWord = new StringBuilder(); // current word, (scTag == null) or comment (when scTag != null)
            ...
            while ((nextChar = ReaderRead()) != -1)
            {
                ...
                {
                    ReaderRead(); // Skip "*" or "+"
                    nComm = new Comment(scCurWord.ToString().Trim(ch, ' ', '\t', '\r', '\n', '*', '+')){
						CommentStartsLine= st.Column < 2,
						Region=new DomRegion(st, new DomLocation(Line,Col)),
						IsDocumentation=isDocumentation};
            ...
            nComm = new Comment( scCurWord.ToString().Trim())
			{CommentType=commentType,CommentStartsLine= st.Column < 2, Region=new DomRegion( st, new DomLocation(Line,Col))};
